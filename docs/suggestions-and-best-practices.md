...
### Total-newbie operator guide (the single front-door doc)
- **Wrote [`docs/operator-guide.md`](operator-guide.md)** as the one comprehensive walk-through a brand-new sysadmin can read top-to-bottom in one sitting. Covers install on 9 platforms (Fedora, RHEL/Rocky/Alma, Debian/Ubuntu, Alpine, Linux ARM64, macOS Intel + Apple Silicon, Windows 10/11/Server, FreeBSD), a 5-minute first-transfer recipe, every flag in every mode with plain-English meaning + default + "when to change," six best-practice presets by scenario (logs/SIEM, batch, large files, non-default MTU, unkeyed, OT/SCADA), per-platform kernel tuning, service definitions for systemd / Windows sc.exe + NSSM / launchd / FreeBSD rc.d, security best practices, day-2 ops, troubleshooting cheat sheet, glossary.
- **Tables for every flag**, columns = flag | default | plain-English meaning | when to change. Mode-by-mode (`tx`, `rx`, `psk`, `vacuum`, `manifest`). A line operator doesn't need to read the man page — the table IS the operator-facing reference.
- **Path-MTU section embeds the exact formula:** `chunk_size_max = path_MTU − 56 − trailer` (trailer = 16 keyed / 32 unkeyed), with a lookup table for the common cases (1500, 1492 PPPoE, 1450 VPN, 1400, 1280 WireGuard). Validated against the actual code: DATAHeaderLen=28, IP/UDP=28, AEAD tag=16, SHA-256 trailer=32. Numbers in the doc match the binary, not folklore.
- **Windows section is honest about UDP performance** — calls out single-process 100–300 Mbps as typical, recommends multi-process scale-out or moving rx to Linux for sustained >300 Mbps. Antivirus exclusions for `inbox\*` and `spool\*` are flagged explicitly because AV scanning every chunk write demolishes throughput.
- **Service definitions are copy-paste runnable on every platform.** systemd unit includes `ProtectSystem=strict` + `ReadWritePaths` + dropped capabilities; Windows section gives both `sc.exe` (no extra tools) and NSSM (better lifecycle) options; launchd plist is full and correct; FreeBSD rc.d follows the standard idiom.
- **Glossary at the end** so a brand-new operator can look up AEAD / FEC / MTU / PSK / SOH / spool / session / vacuum without leaving the doc.

### Line-operator documentation set (runbooks + sizing + preflight)
- **Wrote three artifacts that close the "missing for a line operator" row of the enterprise-roadmap matrix:** [`docs/runbooks.md`](runbooks.md) (7 ranked incidents, each with confirm/mitigate/root-cause/fix/escalate), [`docs/sizing-guide.md`](sizing-guide.md) (host/disk/RAM/network sizing with wire-level math, defaults reference, and a single 30-second sizing answer table at the top), and [`scripts/preflight.sh`](../scripts/preflight.sh) as a stopgap pre-deployment checker until `diode --mode=preflight` ships.
- **Runbook ordering by frequency, not by severity.** A line operator on shift hits the spool-filling and silent-rejection cases far more often than wire-version mismatches. Severity-ordering would put crypto problems first, but those are rare enough that a junior operator never sees them; ordering by "what'll happen tomorrow" is more honest.
- **Sizing-guide section 1 = thirty-second answer table.** Most readers will not read the math; give them a defensible default for four common load profiles up top and treat sections 2-9 as the appendix for the curious. Front-load the answer, not the derivation.
- **Preflight script structure: tx-side checks vs rx-side checks vs common.** A diode deployment has two hosts with very different concerns; lumping them together produces a script no operator wants to read. `--role=tx|rx|both` keeps the output focused.
- **Order-of-magnitude numbers, not benchmarks.** Sizing-guide explicitly says the throughput numbers are architecture-based estimates not measurements, and §9 tells the reader when to bench on their own hardware. False precision in capacity docs erodes trust the first time the operator measures and gets a different answer.
- **GUI is out of scope and called out explicitly.** Runbooks doc's "what's not in this doc" section names "GUI / dashboard → not in scope today; the operator surface is CLI + stderr + the stats line." Avoids the future operator assumption that the GUI is hidden somewhere.
